...
public class MainMenu: MonoBehaviour {
	...
	void Start() {
		...
		List<Hashtable> list = DataBase.getInstance().fetch("SELECT score FROM rating ORDER BY level");

		for (i = 0; i < list.Count; i++) {
			scores[i] = Convert.ToInt32(list[i]["score"]);	// призапуске игры и выходе в главное меню обновляем список рекордов
		}
	}
