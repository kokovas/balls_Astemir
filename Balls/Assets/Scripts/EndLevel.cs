...
public class EndLevel: MonoBehaviour {
	float _scoreLevel=0;								// очки набранные на уровне

	void Start() {
		_scoreLevel=(Score.score*MainMenu.gameTimer);	// количество очков набранных при прохождении уровня

		// если мы набрали больше очков чем в прошлый раз, то перезаписываем рекорд
		if (MainMenu.scores[MainMenu.levelNum-1]<_scoreLevel){
	...
	void OnGUI() {
		...
		GUI.Label(new Rect(Screen.width/2-35, Screen.height/2-170, 70, 20), "** "+_scoreLevel.ToString("f0")+" **");
		...
		if (GUI.Button(new Rect(Screen.width/2-100, Screen.height/2, 200, 40), "Пройти заново уровень "+MainMenu.levelNum)) {
			MainMenu.ballSpeed=MainMenu.levelNum/5;		// 
			MainMenu.gameTimer=MainMenu.ballAmt;		// 
